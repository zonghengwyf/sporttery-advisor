...
async def get_prediction(
    ...
    """获取最近一次对该场次的预测结果，优先返回当前用户自己的预测。"""
    stmt = select(Prediction).where(Prediction.match_id == match_id)
    if current_user:
        stmt = stmt.order_by(
            case((Prediction.user_id == current_user.id, 0), else_=1),
...
async def trigger_analysis_async(
    ...
    """异步触发分析（立即返回，后台执行）。前端轮询 GET /{match_id} 等结果。"""
    match = await db.get(Match, match_id)
    if not match:
        raise HTTPException(status_code=404, detail="赛事不存在")
    ...
    background_tasks.add_task(_run_analysis_background, match_id, llm_cfg, current_user.id)
